...
router.route('/logins').post( async (req, res) => {
    const { username, password, notif_tokens } = req.body;
    console.log('notif_tokens: ', notif_tokens)
    ...
    if(!user){
       ...
       try{
            if(user.hotel_id == password && user.userType =="Admin"){
             ...
             var conditions = {
                _partition: 'project='+user._id,
                notif_tokens: { $nin: [notif_tokens] }
            };
            
            var update = {
                $push: {notif_tokens: notif_tokens}
            }


console.log('user',user)
Additional_Info.findOneAndUpdate(conditions, update, function (err, docs) {
  if (err){
    res.json(user)
  }
  else{
